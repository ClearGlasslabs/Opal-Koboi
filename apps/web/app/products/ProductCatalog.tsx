...
export function ProductCatalog({ products }: { products: ProductRecord[] }) {
  ...
  useEffect(() => { try { const saved = JSON.parse(localStorage.getItem("cg-product-compare") ?? "[]"); if (Array.isArray(saved)) setCompare(saved.filter((id) => products.some((p) => p.id === id)).slice(0, MAX_COMPARE)); } catch { localStorage.removeItem("cg-product-compare"); } trackProductEvent("catalog_view", { surface: "catalog" }); }, [products]);
